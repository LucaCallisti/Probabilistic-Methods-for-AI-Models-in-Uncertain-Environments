...
* **Objective Function:** The agent maximizes the entropy-regularized expected reward, where the *temperature parameter* $\alpha$ balances the trade-off between exploitation and exploration. $`\pi^{*}=\arg\max_{\pi} \mathbb{E}_{\tau \sim \pi}\big[ \sum_{t=0}^{\infty}  \gamma^t (R(x_t, a_t, x_{t+1}) + \alpha H(\pi(\cdot|x_t))) \big]`$
